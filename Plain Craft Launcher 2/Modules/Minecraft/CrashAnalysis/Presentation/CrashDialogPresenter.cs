...
internal sealed class CrashDialogPresenter(CrashAnalysisContext context)
{
        ...
        List<string>? extraFiles)
    {
        ...
        var uploadButtonText = string.IsNullOrWhiteSpace(context.LogAll)
            ? ""
            : Lang.Text("Log.Upload.Button");
        Action? uploadButtonAction = string.IsNullOrWhiteSpace(context.LogAll)
            ? null
            : () => LogUploadService.UploadAndCopyAsync(context.LogAll);
        ...
        var selectedButton = MsgBoxWrapper.ShowWithCustomButtons(
            ...
            new MsgBoxButtonInfo(uploadButtonText, 4, uploadButtonAction));
