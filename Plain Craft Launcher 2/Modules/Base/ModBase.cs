...
public static class ModBase
{
    ...
    private static void UploadLauncherLogFromMessage(string message)
    {
        LogUploadService.UploadAndCopyAsync(GetLauncherLogUploadText(message));
    }

    private static string GetLauncherLogUploadText(string message)
    {
        var builder = new StringBuilder();
        ...
        try
        {
            ...
            {
                builder.AppendLine("===== PCL Launcher Log =====");
                builder.AppendLine(File.ReadAllText(currentLogFile, Encoding.UTF8));
            }
        }
        catch (Exception ex)
        {
            LogWrapper.Warn(ex, "LogUpload", "读取当前日志失败");
        }
    ...
    public static void Log(string text, LogLevel level = LogLevel.Normal, string title = "出现错误")
    {
        ...
        switch (level)
        {
            ...
            {
                ModMain.MyMsgBox(text, title, button3: Lang.Text("Log.Upload.Button"), isWarn: true,
                    button3Action: () => UploadLauncherLogFromMessage(text));
                break;
            }
            case LogLevel.Feedback:
            {
                if (CanFeedback(false))
                {
                    if (ModMain.MyMsgBox(text + "\r\n" + "\r\n" + "是否反馈此问题？如果不反馈，这个问题可能永远无法得到解决！",
                            title, "反馈", Lang.Text("Common.Action.Cancel"), Lang.Text("Log.Upload.Button"), isWarn: true,
                            button3Action: () => UploadLauncherLogFromMessage(text)) == 1)
                        Feedback(false, true);
                }
                else
                {
                    ModMain.MyMsgBox(text + "\r\n" + "\r\n" + "将 PCL 更新至最新版或许可以解决这个问题……", title,
                        button3: Lang.Text("Log.Upload.Button"), isWarn: true,
                        button3Action: () => UploadLauncherLogFromMessage(text));
                }
    ...
    public static void Log(Exception ex, string desc, LogLevel level = LogLevel.Debug, string title = "出现错误")
    {
        ...
        switch (level)
        {
            ...
            case LogLevel.Msgbox:
            {
                ModMain.MyMsgBox(exFull, title, button3: Lang.Text("Log.Upload.Button"), isWarn: true,
                    button3Action: () => UploadLauncherLogFromMessage(desc + "：\r\n" + ex));
                break;
            }
            case LogLevel.Feedback:
            {
