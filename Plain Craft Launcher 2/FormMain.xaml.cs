...
public partial class FormMain
{
            ...
            try
            {
                ...
                if (Environment.GetEnvironmentVariable("PCL_DISABLE_DEBUG_HINT") is null)
                {
                    ...
                    ModMain.MyMsgBox(
                        ...
                        {
                            ModBase.OpenWebsite("https://github.com/PCL-Nex-Developer/PCL2-Nex/releases/latest");
                            EndProgram(false);
                        });
                }
    ...
    private void SetTopNavigationChecked(PageType pageType, bool raiseByMouse)
    {
        var tagText = ((int)pageType).ToString();
        foreach (UIElement child in PanTitleSelect.Children)
        {
            if (child is MyRadioButton button && string.Equals(button.Tag?.ToString(), tagText, StringComparison.OrdinalIgnoreCase))
            {
                button.SetChecked(true, true, raiseByMouse);
                return;
            }
        }
    }
    ...
    public void PageChange(PageStackData stack, PageSubType subType = PageSubType.Default)
    {
        if (string.IsNullOrEmpty(PageNameGet(stack)))
        {
            ...
            SetTopNavigationChecked(stack.page, string.IsNullOrEmpty(PageNameGet(pageCurrent)));
    ...
    private void PageChangeActual(PageStackData stack, PageSubType subType)
    {
        ...
        try
        {
            ...
            switch (stack.page)
            {
                ...
                case PageType.Plugins: // 插件
                    {
                        ModMain.frmPluginsLeft ??= new PagePluginsLeft();
                        subType = ModMain.frmPluginsLeft.pageID;
                        PageChangeAnim(ModMain.frmPluginsLeft, (FrameworkElement)ModMain.frmPluginsLeft.PageGet(subType));
                        break;
                    }
    ...
    // 更新重启
    public void RefreshRestartButton(bool ribble = false)
    {
        if (!UpdateManager.isUpdateWaitingRestart)
            BtnExtraUpdateRestart.ToolTip = PluginEnablementService.HasPendingRestartChanges()
                ? Lang.Text("Main.Extra.Restart.PluginToolTip")
                : Lang.Text("Main.Extra.Restart.ToolTip");
        BtnExtraUpdateRestart.ShowRefresh();
        if (ribble && BtnExtraUpdateRestart.Show)
            BtnExtraUpdateRestart.Ribble();
    }
    ...
    {
        return UpdateManager.isUpdateWaitingRestart || PluginEnablementService.HasPendingRestartChanges();
    }
