...
public static class ImageConverter
{
    ...
    public static MemoryStream FromWebpToPng(this Stream input, int frameIndex = 0)
    {
        try
        {
            return NormalizeToPng(input, frameIndex);
        }
        ...
        {
            throw new NotSupportedException("转换 WebP 到 PNG 失败：输入可能不是有效的 WebP，或格式不受支持。", ex);
        }
    }

    /// <summary>
    /// 将任意受支持的位图统一为无颜色配置的 8 位 RGBA PNG。
    /// </summary>
    public static MemoryStream NormalizeToPng(Stream input, int frameIndex = 0)
    {
        ...
        try
        {
            ...
            frame.Save(output, CreateCanonicalPngEncoder());
            output.Position = 0;
            return output;
        }
        finally
