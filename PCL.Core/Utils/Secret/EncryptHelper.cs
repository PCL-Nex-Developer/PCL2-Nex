...
{
    private static readonly byte[] Key = "PCL Nex Encryption Key"u8.ToArray();
...
#pragma warning restore CS0612,CS0618 // Type or member is obsolete
    ...
    private static byte[] _GetKey()
    {
        ...
        if (File.Exists(keyFile))
        {
            ...
            return data.Version switch
            {
                ...
                2 => _UnprotectWithFallback(data.Data),
        ...
        var storeData = _ProtectWithFallback(randomKey);
    ...
    /// <summary>
    /// 优先使用 CNG DPAPI 保护密钥；当当前平台的 CNG LOCAL provider 不支持 optional entropy
    /// （抛 <see cref="PlatformNotSupportedException"/>）时，退回到标准 DPAPI 并以 Version 1 存储。<br/>
    /// 读取侧的 Version 1/2 分支已分别对应两种实现，因此跨机器兼容性得以保留。
    /// </summary>
    private static byte[] _ProtectWithFallback(byte[] key)
    {
        try
        {
            return EncryptionData.ToBytes(new EncryptionData
            {
                Version = 2,
                Data = CngProtectedData.Protect(key, Key, CngDataProtectionScope.CurrentUser)
            });
        }
        catch (PlatformNotSupportedException)
        {
            ...
            {
                Version = 1,
                Data = ProtectedData.Protect(key, Key, DataProtectionScope.CurrentUser)
            });
        }
    }

    /// <summary>
    /// 优先使用 CNG DPAPI 解除保护；平台不支持时退回标准 DPAPI。<br/>
    /// 仅在 CNG 实现本身就不可用的退化场景下生效，避免静默掩盖真正损坏的密钥。
    /// </summary>
    private static byte[] _UnprotectWithFallback(byte[] data)
    {
        try
        {
            return CngProtectedData.Unprotect(data, Key, CngDataProtectionScope.CurrentUser);
        }
        catch (PlatformNotSupportedException)
        {
