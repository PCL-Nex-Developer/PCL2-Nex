...
public static partial class KernelInterop
{
    ...
    /// </summary>
    public static uint CurrentNativeThreadId => OperatingSystem.IsWindows()
        ? _GetCurrentThreadId()
    ...
    public static void ExitProcess(int statusCode = 0)
    {
        if (OperatingSystem.IsWindows())
            _ExitProcess((uint)statusCode);
        else
    ...
    {
        if (!OperatingSystem.IsWindows())
            throw new PlatformNotSupportedException("当前平台不支持查询命名管道客户端进程 ID。");
    ...
    {
        return GetPhysicalMemoryBytes().Available;
    }
    ...
    public static (ulong Total, ulong Available) GetPhysicalMemoryBytes()
    {
        ...
        if (OperatingSystem.IsLinux() && TryReadLinuxMemoryInfo(out var memory))
            return memory;

        var gcInfo = GC.GetGCMemoryInfo();
        var total = (ulong)Math.Max(gcInfo.TotalAvailableMemoryBytes, GC.GetTotalMemory(false));
        var used = (ulong)Math.Max(GC.GetTotalMemory(false), 0);
        return (total, total > used ? total - used : 0);
    }
    ...
    public static double GetMemoryLoadPercent()
    {
        ...
        return total == 0 ? 0 : (total - available) * 100d / total;
    }
    ...
    {
        if (OperatingSystem.IsWindows()) _FreeConsole();
    }
    ...
    public static nint GetConsoleWindow() => OperatingSystem.IsWindows() ? _GetConsoleWindow() : nint.Zero;

    private static bool TryReadLinuxMemoryInfo(out (ulong Total, ulong Available) memory)
    {
        memory = default;
        try
        {
            ...
            memory = (total, Math.Min(available, total));
            return true;
        }
        ...
        static ulong ParseKilobytes(string line)
        {
            var parts = line.Split(' ', StringSplitOptions.RemoveEmptyEntries);
            return parts.Length >= 2 && ulong.TryParse(parts[1], out var value) ? value * 1024 : 0;
        }
    }
}
