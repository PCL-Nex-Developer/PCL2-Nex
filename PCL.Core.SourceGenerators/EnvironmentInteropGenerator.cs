...
#pragma warning restore RS1035
            var secretPairs = envs.Contains("PCL_WRITE_SECRET") ? (
                ...
                let secretKey = key.StartsWith("PCL_", StringComparison.Ordinal) && key != "PCL_WRITE_SECRET"
                    ? key.Substring(4)
                    : null
                where secretKey is not null
                select (Key: secretKey!, Value: value!)
            ).GroupBy(pair => pair.Key).Select(group => group.First()) : [];
            return secretPairs;
        });
